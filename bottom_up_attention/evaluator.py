...
class Evaluator(nn.Module):
    ...
    def __init__(self, batch_size=100, output_directory='.'):
        super().__init__()

        self.output_directory = output_directory
        self.batch_size = batch_size

    def _sample_captioning(self, model, dataset):
        ...
        # output directory
        captioning_directory = os.path.join(self.output_directory,
                                            'captioning')
        os.makedirs(captioning_directory, exist_ok=True)
        ...
        dataloader = DataLoader(dataset,
                                ...
                                shuffle=True,
                                num_workers=1,
                                pin_memory=model.cuda_available)

        # Lists to store references (true captions), and hypothesis
        # (prediction) for each image If for n images, we have n hypotheses,
        # and references a, b, c... for each image, we need - references =
        # [[ref1a, ref1b, ref1c], [ref2a, ref2b], ...], hypotheses = [hyp1,
        # hyp2, ...]
        references = list()
        hypotheses = list()
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                ...
                # Start decoding
                step = 1
                h1, c1 = model.init_hidden_state(
                    k)  # (batch_size, decoder_dim)
                h2, c2 = model.init_hidden_state(k)

                # s is a number less than or equal to k, because sequences are removed from this process once they hit <end>
                while True:
                    embeddings = model.embedding(k_prev_words).squeeze(
                        1)  # (s, embed_dim)
                    ...
                    incomplete_inds = [
                        ...
                        if next_word != dataset.word_map['<end>']
                    ]
                    complete_inds = list(
                    ...
                    map(
                        lambda c: [
                            rev_word_map[w] for w in c if w not in {
                                dataset.word_map['<start>'], dataset.word_map[
                                    '<end>'], dataset.word_map['<pad>']
                            }
                ...
                hypothesis = ([
                    rev_word_map[w] for w in seq if w not in {
                        dataset.word_map['<start>'], dataset.word_map['<end>'],
                        dataset.word_map['<pad>']
                    }
                ])
                ...
                for i in image_id:
                    image_file = os.path.join(dataset.image_prefix, i)
                    ...
                    font = ImageFont.truetype(
                        "/usr/share/fonts/dejavu/DejaVuSans.ttf", 18)
                    draw = ImageDraw.Draw(pad_im)
                    draw.text((0, im_height + 30),
                              hypothesis, (255, 255, 255),
                              font=font)
                    draw = ImageDraw.Draw(pad_im)
                    pad_im.save(
                        os.path.join(captioning_directory, i + '_pred.jpg'))
            ...
            with open(
                    os.path.join(self.output_directory,
                                 'metrics_captioning.txt'), 'w') as f:
                f.writelines([
                    'BLEU-4: ',
                    str(metrics_dict['Bleu_4']), '\n', 'METEOR: ',
                    str(metrics_dict['METEOR']), '\n', 'ROUGE-L: ',
                    str(metrics_dict['ROUGE_L']), '\n', 'CIDEr: ',
                    str(metrics_dict['CIDEr']), '\n'
                ])

    def _sample_vqa(self, model, dataset):
        # sample images using specified snapshot vqa model
        ...
        # output directory
        vqa_directory = os.path.join(self.output_directory, 'vqa')
        os.makedirs(vqa_directory, exist_ok=True)
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                batch_score = model.compute_score_with_logits(logits,
                                                              answers).sum()
                score += batch_score
                ...
                for j in range(num_batch_images):
                    ...
                    # image filename
                    image_file = dataset.image_prefix + str(
                        int(image_id)).zfill(12) + '.jpg'
                    im = Image.open(image_file)
                    # size of image
                    im_height = im.height
                    im_width = im.width
                    ...
                    draw = ImageDraw.Draw(pad_im)
                    draw.text((0, im_height + 30),
                              questions[j], (255, 255, 255),
                              font=font)
                    draw.text((0, im_height + 60),
                              answer, (0, 255, 0),
                              font=font)
                    draw = ImageDraw.Draw(pad_im)
                    pad_im.save(
                        os.path.join(
                            vqa_directory,
                            str(int(image_id)).zfill(12) + '_pred.jpg'))

            score = score / len(dataloader.dataset)
            upper_bound = upper_bound / len(dataloader.dataset)

            with open(os.path.join(self.output_directory, 'metrics_vqa.txt'),
                      'w') as f:
                f.writelines([
                    'Score: ',
                    str(score), '\n', 'Upper Bound: ',
    ...
    def sample(self, task, model, dataset):
        if not os.path.exists(self.output_directory):
            os.makedirs(self.output_directory, exist_ok=True)
        (self._sample_captioning
         if task == 'captioning' else self._sample_vqa)(model, dataset)
